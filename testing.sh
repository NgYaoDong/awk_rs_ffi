...
test x"$ECHO" != x"" || {
	...
	test x"`echo -ne`" = x"" || {
		...
		ECHO="$PWD/src_c/echo-ne"
		test -x "$ECHO" || {
			gcc -Os -o "$ECHO" ./src_c/echo.c || exit 1
		}
	}
	export ECHO
}
...
testing()
{
  ...
  # print the content of file actual
  # cat actual
  # open file `actual` and remove the line starting with `[pkalloc]`
  sed -i '/^\[pkalloc\]/d' actual
  # cat actual
  RETVAL=$?
